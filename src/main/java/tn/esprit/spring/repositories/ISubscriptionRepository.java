...
public interface ISubscriptionRepository extends JpaRepository<Subscription, Long> { // Changez ici
    ...
    @Query("select (sum(s.price))/(count(s)) from Subscription s where s.typeSub = :typeSub") // Correction ici
    Float recurringRevenueByTypeSubEquals(@Param("typeSub") TypeSubscription typeSub); // Correction ici
}
